        ...
        <div>
            ...
            <div className="bg-white border border-solid border-[#E5E5E5] rounded-[24px] px-6 py-4 mb-4">
                <input
                    ...
                    className="h-[48px] px-4 border border-solid border-[#E5E5E5] rounded-full text-[14px] outline-none focus:border-[#171717] transition-colors w-full sm:w-[280px]"
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtered.map(b => (
                    <div key={b.id} className="bg-white border border-solid border-[#E5E5E5] rounded-[24px] overflow-hidden hover:shadow-md transition-shadow">
            ...
            {editing && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(23,23,23,0.5)', backdropFilter: 'blur(4px)' }}>
                    <div className="bg-white rounded-[24px] w-full max-w-[440px]">
                        <div className="px-6 py-5 border-b border-solid border-[#E5E5E5] flex items-center justify-between">
                            <h4 className="font-Helvetica font-normal text-[20px]">Edit Post</h4>
                        ...
                        <div className="px-6 py-5 flex flex-col gap-4">
                            <div>
                                ...
                                <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="w-full h-[48px] px-4 border border-solid border-[#E5E5E5] rounded-[8px] text-[14px] outline-none focus:border-[#171717] transition-colors" />
                            </div>
                            <div>
                                <label className="block text-[13px] font-medium mb-1">Tag</label>
                                <input type="text" value={form.tag} onChange={e => setForm({ ...form, tag: e.target.value })} className="w-full h-[48px] px-4 border border-solid border-[#E5E5E5] rounded-[8px] text-[14px] outline-none focus:border-[#171717] transition-colors" />
                            </div>
                            <div>
                                <label className="block text-[13px] font-medium mb-1">Author</label>
                                <input type="text" value={form.by} onChange={e => setForm({ ...form, by: e.target.value })} className="w-full h-[48px] px-4 border border-solid border-[#E5E5E5] rounded-[8px] text-[14px] outline-none focus:border-[#171717] transition-colors" />
                            </div>
                            <div>
                                <label className="block text-[13px] font-medium mb-1">Status</label>
                                <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full h-[48px] px-4 border border-solid border-[#E5E5E5] rounded-[8px] text-[14px] outline-none focus:border-[#171717] transition-colors bg-white">
                                    <option>Published</option>
                                    <option>Draft</option>
                                </select>
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => setEditing(null)} className="flex-1 h-[48px] border border-solid border-[#E5E5E5] rounded-full text-[14px] font-medium hover:border-black transition-colors">Cancel</button>
                                <button onClick={handleSave} className="flex-1 h-[48px] bg-[#171717] text-white rounded-full text-[14px] font-medium hover:bg-orange transition-colors">Save</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {confirmDelete && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(23,23,23,0.5)', backdropFilter: 'blur(4px)' }}>
                    <div className="bg-white rounded-[24px] w-full max-w-[360px] px-6 py-6 text-center">
                        ...
                        <h4 className="font-Helvetica font-normal text-[20px] mb-2">Delete Post?</h4>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={() => { deleteBlog(confirmDelete.id); setConfirmDelete(null) }} className="flex-1 h-[48px] bg-red-500 text-white rounded-full text-[14px] font-medium hover:bg-red-600 transition-colors">Delete</button>
                        </div>
                    </div>
                </div>
            )}
